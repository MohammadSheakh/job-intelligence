...
export async function listOtherCompanies(limit?: number): Promise<EnrichmentCompany[]> {
  ...
  const limitSql = typeof limit === 'number' && limit > 0 ? (params.push(limit), `LIMIT $${params.length}`) : '';
...
export async function addCompanyCategories(companyId: string, names: string[], source: string): Promise<void> {
  if (!names.length) return;
  await db.query(`
