...
async function trackingTableExists(): Promise<boolean> {
  ...
  const result = await db.query<{ exists: boolean }>(
    `SELECT to_regclass('public.candidate_company_state') IS NOT NULL AS exists`,
  );
  trackingTableCache = Boolean(result.rows[0]?.exists);
  return trackingTableCache;
}

export async function getCandidateCompanyStateMap(
  candidateId: number,
): Promise<Map<string, 'PLANNING' | 'APPLIED' | 'EXCLUDED'>> {
...
export async function getTrackingCounts(
  candidateId: number,
): Promise<{ planning: number; applied: number; excluded: number }> {
...
export async function searchCompaniesForCandidate(
  candidateId: number,
  input: { q?: string; category?: string; limit?: number },
): Promise<PortalCompanyRow[]> {
  ...
  if (input.q?.trim()) {
    ...
    where.push(
      `(c.name ILIKE $${params.length} OR COALESCE(c.location,'') ILIKE $${params.length} OR COALESCE(c.tech_stack,'') ILIKE $${params.length})`,
    );
  }
  if (input.category?.trim()) {
    params.push(input.category.trim());
    where.push(
      `EXISTS(SELECT 1 FROM company_categories ccx JOIN categories cx ON cx.id=ccx.category_id WHERE ccx.company_id=c.id AND cx.name=$${params.length})`,
    );
  }
  let trackingJoin = '';
  ...
  const result = await db.query<{
    ...
    status: 'PLANNING' | 'APPLIED' | 'EXCLUDED' | null;
  ...
  return result.rows.map((r) => ({
    ...
    reapplyCount: Number(r.reapply_count ?? 0),
    notes: r.notes,
  }));
}
...
export async function listTrackedCompanies(
  ...
  return result.rows.map((r) => ({
    ...
    trackingStatus: r.status,
    lastAppliedAt: r.last_applied_at,
    reapplyCount: Number(r.reapply_count),
    notes: r.notes,
  }));
}

export async function saveCompanyTracking(input: {
  candidateId: number;
  companyId: string;
  status: 'PLANNING' | 'APPLIED' | 'EXCLUDED';
  ...
  if (!(await trackingTableExists()))
    throw new Error(
      'Company pipeline storage is not migrated yet. Run sql/006_candidate_portal.sql.',
    );
...
export async function removeCompanyTracking(candidateId: number, companyId: string): Promise<void> {
  if (!(await trackingTableExists())) return;
  await db.query('DELETE FROM candidate_company_state WHERE candidate_id=$1 AND company_id=$2', [
    candidateId,
    companyId,
  ]);
}
...
export async function getPortalCandidateProfile(
  candidateId: number,
...
export async function listPortalCategoryCatalog(): Promise<PortalCategory[]> {
  const result = await db.query<PortalCategory>(
    `SELECT name,type FROM categories WHERE name<>'Other' ORDER BY type,name`,
  );
  return result.rows;
}
...
export async function updatePortalCandidateProfile(
  candidateId: number,
  input: {
    name: string;
    expertise?: string;
  ...
  if (!Number.isFinite(threshold) || threshold < 0 || threshold > 100)
    throw new Error('Match threshold must be between 0 and 100.');

  const allowedModes = new Set(['Remote', 'Hybrid', 'On-site']);
  const workModes = [
    ...new Set((input.preferredWorkModes ?? []).filter((x) => allowedModes.has(x))),
  ];

  const catalog = await listPortalCategoryCatalog();
  const allowedCategories = new Set(catalog.map((c) => c.name));
  const excluded = [
    ...new Set((input.excludedCategories ?? []).filter((x) => allowedCategories.has(x))),
  ];
  const excludedSet = new Set(excluded);
  const preferred = [
    ...new Set(
      (input.preferredCategories ?? []).filter(
        (x) => allowedCategories.has(x) && !excludedSet.has(x),
      ),
    ),
  ];
  ...
  `,
    [
      candidateId,
      name,
      cleanText(input.expertise, 500),
      cleanText(input.skills, 1000),
      cleanText(input.experienceLevel, 80),
      cleanText(input.preferredLocations, 1000),
      cleanText(input.excludedLocations, 1000),
