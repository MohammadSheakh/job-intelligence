...
import {
  ...
  findOrBindGoogleCandidate,
  getCandidateById,
  setCandidatePassword,
} from '../auth/candidate-auth.js';
...
import {
  getPortalCandidateProfile,
  getTrackingCounts,
  listPortalCategories,
  listPortalCategoryCatalog,
  listTrackedCompanies,
  removeCompanyTracking,
  saveCompanyTracking,
  searchCompaniesForCandidate,
  updatePortalCandidateProfile,
} from './repository.js';
...
function cookie(
  name: string,
  value: string,
  opts: { maxAge?: number; expires?: Date; httpOnly?: boolean } = {},
): string {
  const bits = [
...
function layout(title: string, body: string, identity?: { name: string }): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} · Job Intelligence</title><style>
${FERIO_CSS}
...
function trackingPill(status: string | null): string {
  if (!status) return '—';
  if (status === 'APPLIED') return '<span class="status success">Applied</span>';
  if (status === 'PLANNING') return '<span class="status pending">Planning</span>';
  if (status === 'EXCLUDED') return '<span class="status error">Blacklist</span>';
...
export async function handlePortalRequest(
  ...
  try {
    if (req.method === 'GET' && url.pathname === '/portal/login') {
      const { user } = await identity(req);
      if (user)
        return redirect(res, user.mustChangePassword ? '/portal/change-password' : '/portal');
      return send(res, 200, loginPage(url.searchParams.get('error') ?? ''));
    }
    if (req.method === 'POST' && url.pathname === '/portal/login') {
      const form = await readForm(req);
      const user = await authenticateCandidate(form.get('email') ?? '', form.get('password') ?? '');
      if (!user) return send(res, 401, loginPage('Invalid email or password.'));
      const session = createSignedSession(user.id);
      return redirect(res, user.mustChangePassword ? '/portal/change-password' : '/portal', {
        'set-cookie': cookie(sessionCookie, session.token, { expires: session.expiresAt }),
      });
    }
    if (req.method === 'GET' && url.pathname === '/auth/google/start') {
      if (!googleLoginEnabled())
        return redirect(res, '/portal/login?error=Google+login+is+not+configured');
      const state = randomBytes(24).toString('base64url');
      res.writeHead(302, {
        location: googleAuthorizationUrl(state),
        'set-cookie': cookie(googleStateCookie, state, { maxAge: 600 }),
      });
      res.end();
      return;
    }
    if (req.method === 'GET' && url.pathname === '/auth/google/callback') {
      ...
      if (!state || state !== cookies[googleStateCookie] || !code)
        return redirect(res, '/portal/login?error=Google+login+verification+failed', {
          'set-cookie': cookie(googleStateCookie, '', { maxAge: 0 }),
        });
      const profile = await exchangeGoogleCode(code);
      const user = await findOrBindGoogleCandidate({ email: profile.email, sub: profile.sub });
      if (!user)
        return redirect(
          res,
          '/portal/login?error=Your+Google+email+is+not+an+active+candidate+account',
          { 'set-cookie': cookie(googleStateCookie, '', { maxAge: 0 }) },
        );
      const session = createSignedSession(user.id);
      return redirect(res, user.mustChangePassword ? '/portal/change-password' : '/portal', {
        'set-cookie': [
          cookie(sessionCookie, session.token, { expires: session.expiresAt }),
          cookie(googleStateCookie, '', { maxAge: 0 }),
        ],
      });
    }

    const { user } = await identity(req);
    if (!user) return redirect(res, '/portal/login');
    if (req.method === 'POST' && url.pathname === '/portal/logout')
      return redirect(res, '/portal/login', {
        'set-cookie': cookie(sessionCookie, '', { maxAge: 0 }),
      });
    ...
    if (req.method === 'POST' && url.pathname === '/portal/change-password') {
      ...
      await setCandidatePassword(user.id, password, { mustChange: false });
    ...
    if (req.method === 'GET' && url.pathname === '/portal') {
      const [counts, profile, settings, recommendations] = await Promise.all([
        getTrackingCounts(user.id),
        getPortalCandidateProfile(user.id),
        getSettings(),
        getPortalRecommendations(user.id, 8),
      ]);
      const usage = await getQuickSearchUsage(
        user.id,
      ...
      const recommendationRows = recommendations
        .map(
          (job) =>
            `<div class="job-row"><div><div class="job-title">${esc(job.title)}</div><div class="job-meta">${esc(job.companyName)}${job.location ? ` · ${esc(job.location)}` : ''}${job.workMode ? ` · ${esc(job.workMode)}` : ''}</div>${job.categories.length ? `<div class="chips">${job.categories.map((c) => `<span class="chip">${esc(c)}</span>`).join('')}</div>` : ''}<div class="job-reasons">${job.reasons.length ? job.reasons.map(esc).join(' · ') : 'Matched to your profile preferences.'}</div></div><div class="job-actions"><span class="score">${job.score}%</span>${job.applicationUrl ? `<a class="btn" href="${esc(job.applicationUrl)}" target="_blank" rel="noreferrer">Apply</a>` : ''}${companyActions(job.companyId, job.trackingStatus, '/portal')}</div></div>`,
        )
        .join('');
    ...
    if (req.method === 'GET' && url.pathname === '/portal/profile') {
      ...
      const categorySections = groups
        .map(([type, label]) => {
          ...
          const preferredBoxes = rows
            .map(
              (c) =>
                `<label class="check"><input type="checkbox" name="preferred_categories" value="${esc(c.name)}" ${preferred.has(c.name) ? 'checked' : ''}><span>${esc(c.name)}</span></label>`,
            )
            .join('');
          const excludedBoxes = rows
            .map(
              (c) =>
                `<label class="check"><input type="checkbox" name="excluded_categories" value="${esc(c.name)}" ${excluded.has(c.name) ? 'checked' : ''}><span>${esc(c.name)}</span></label>`,
            )
            .join('');
        ...
        .join('');
      const experienceOptions = ['', 'Intern', 'Junior', 'Mid', 'Senior', 'Lead']
        .map(
          (x) =>
            `<option value="${esc(x)}" ${x.toLowerCase() === (profile.experienceLevel ?? '').toLowerCase() ? 'selected' : ''}>${esc(x || 'Not specified')}</option>`,
        )
        .join('');
      const saved =
        url.searchParams.get('saved') === '1'
          ? '<div class="notice">Profile updated. New matching and Quick Search runs will use these preferences.</div>'
          : '';
    ...
    if (req.method === 'POST' && url.pathname === '/portal/quick-search') {
      const form = await readForm(req);
      const mode = (form.get('mode') ?? 'STANDARD') === 'AI' ? 'AI' : 'STANDARD';
      const result = await runCandidateQuickSearch(user.id, mode);
      const rows = result.matches
        .map(
          (m) =>
            `<tr><td><strong>${esc(m.title)}</strong><div class="muted">${esc(m.companyName)}${m.location ? ` · ${esc(m.location)}` : ''}</div><div class="muted">${m.reasons.map(esc).join(' · ')}</div></td><td><span class="score">${m.score}%</span>${m.aiUsed ? '<div class="chip">AI refined</div>' : ''}</td><td><div class="actions">${m.applicationUrl ? `<a class="btn" href="${esc(m.applicationUrl)}" target="_blank" rel="noreferrer">Apply</a>` : ''}${companyActions(m.companyId, null, '/portal')}</div></td></tr>`,
        )
        .join('');
    ...
    if (req.method === 'GET' && url.pathname === '/portal/pipeline') {
      ...
      const rows = await listTrackedCompanies(user.id, status || undefined);
      const trs = rows
        .map(
          (c) =>
    ...
    if (req.method === 'POST' && url.pathname === '/portal/company-state') {
      ...
      if (!['PLANNING', 'APPLIED', 'EXCLUDED'].includes(status))
        throw new Error('Invalid company status.');
      await saveCompanyTracking({
        candidateId: user.id,
        companyId: form.get('company_id') ?? '',
        status: status as 'PLANNING' | 'APPLIED' | 'EXCLUDED',
        lastAppliedAt: form.get('last_applied_at') ?? '',
        reapplyCount: Number(form.get('reapply_count') ?? '0') || 0,
      ...
      const safeReturn =
        returnTo.startsWith('/portal') && !returnTo.startsWith('//')
          ? returnTo
          : req.headers.referer?.includes('/portal/pipeline')
            ? '/portal/pipeline'
            : '/portal/companies';
      return redirect(res, safeReturn);
    }
    if (req.method === 'POST' && url.pathname === '/portal/company-remove') {
      const form = await readForm(req);
      await removeCompanyTracking(user.id, form.get('company_id') ?? '');
      return redirect(res, '/portal/pipeline');
    }
    return send(
      res,
      404,
      layout('Not found', '<div class="card">Candidate page not found.</div>', user),
    );
  } catch (error) {
    console.error(error);
    const message = error instanceof Error ? error.message : String(error);
    return send(res, 500, layout('Error', `<div class="notice error">${esc(message)}</div>`));
  }
}
