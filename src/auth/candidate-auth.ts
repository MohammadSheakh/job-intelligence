...
export async function ensureCandidateDefaultPassword(
  candidateId: number,
  password = env.defaultCandidatePassword,
): Promise<void> {
  const existing = await db.query<{ password_hash: string | null }>(
    'SELECT password_hash FROM candidate_auth WHERE candidate_id=$1',
    [candidateId],
  );
...
export async function findOrBindGoogleCandidate(input: {
  email: string;
  sub: string;
}): Promise<CandidateIdentity | null> {
