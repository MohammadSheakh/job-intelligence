  ...
  providers: [
    {
      ...
      useValue: {
        app: {
          ...
          defaultCandidatePassword: 'default-test-password',
        },
      },
    },
  ],
...
const candidateInput = {
  ...
  email: 'Admin-Candidate@example.test',
  active: true,
  minimumMatchScore: 60,
  preferredCategories: [' NestJS ', 'Logistics', 'NestJS'],
...
const settingsInput = {
  ...
  emailEnabled: true,
  quickSearchDailyLimit: 4,
  quickSearchAiDailyLimit: 2,
  quickSearchCompanyLimit: 9,
};
...
describe('Admin APIs with disposable PostgreSQL', () => {
  ...
  it.each(['candidates', 'companies', 'categories', 'jobs', 'settings', 'crawl-logs'])(
    'protects admin %s with Basic authentication',
    async (route) => {
  ...
  it('creates normalized candidates with a hashed default password and no secret exposure', async () => {
    const response = await admin(api().post('/api/v1/admin/candidates'))
      .send(candidateInput)
      .expect(201);
    const id = response.body.id;
    const auth = await prisma.candidateAuth.findUniqueOrThrow({
  ...
  it('preserves passwords on profile edits and forces change after explicit reset', async () => {
    const created = await admin(api().post('/api/v1/admin/candidates'))
      .send({ ...candidateInput, newPassword: 'explicit-password' })
      .expect(201);
    const id = created.body.id;
    const before = await prisma.candidateAuth.update({
      where: { candidateId: BigInt(id) },
      data: { mustChangePassword: false },
    });
    await admin(api().put(`/api/v1/admin/candidates/${id}`))
      .send({ ...candidateInput, name: 'Updated' })
      .expect(200);
    expect(
      await prisma.candidateAuth.findUnique({ where: { candidateId: BigInt(id) } }),
    ).toMatchObject({ passwordHash: before.passwordHash, mustChangePassword: false });
    await admin(api().put(`/api/v1/admin/candidates/${id}`))
      .send({ ...candidateInput, newPassword: 'reset-password' })
      .expect(200);
    const after = await prisma.candidateAuth.findUniqueOrThrow({
      where: { candidateId: BigInt(id) },
    });
  ...
  it('rejects duplicate emails on create and update without modifying profiles', async () => {
    const first = await admin(api().post('/api/v1/admin/candidates'))
      .send(candidateInput)
      .expect(201);
    await admin(api().post('/api/v1/admin/candidates'))
      .send({ ...candidateInput, email: candidateInput.email.toUpperCase() })
      .expect(409);
    const second = await admin(api().post('/api/v1/admin/candidates'))
      .send({ ...candidateInput, email: 'admin-second@example.test' })
      .expect(201);
    await admin(api().put(`/api/v1/admin/candidates/${second.body.id}`))
      .send({ ...candidateInput, name: 'Should not persist' })
      .expect(409);
    expect(
      await prisma.candidate.count({
        where: { id: { in: [BigInt(first.body.id), BigInt(second.body.id)] } },
      }),
    ).toBe(2);
    expect(
      (await prisma.candidate.findUniqueOrThrow({ where: { id: BigInt(second.body.id) } })).name,
    ).toBe('Admin Candidate');
  });

  it.each(['abc', '0', '-1', '0x10', '9223372036854775808'])(
    'rejects invalid candidate IDs (%s)',
    async (id) => {
  ...
  it('returns 404 for missing candidates and rejects invalid candidate input', async () => {
    ...
    await admin(api().post('/api/v1/admin/candidates'))
      .send({ ...candidateInput, minimumMatchScore: 101 })
      .expect(400);
    await admin(api().post('/api/v1/admin/candidates'))
      .send({ ...candidateInput, newPassword: 'short' })
      .expect(400);
    expect(await prisma.candidate.count({ where: { email: 'admin-candidate@example.test' } })).toBe(
      0,
    );
  });
  ...
  it('rolls back candidate creation when authentication persistence fails', async () => {
    ...
    try {
      ...
      expect(
        await prisma.candidate.count({ where: { email: 'admin-candidate@example.test' } }),
      ).toBe(0);
    } finally {
      await prisma.$executeRaw`ALTER TABLE candidate_auth DROP CONSTRAINT test_reject_password`;
    }
  });
  ...
  it('rolls back profile edits when password reset persistence fails', async () => {
    ...
    try {
      await admin(api().put(`/api/v1/admin/candidates/${id}`))
        .send({ ...candidateInput, name: 'Should roll back', newPassword: 'reset-password' })
        .expect(500);
      expect((await prisma.candidate.findUniqueOrThrow({ where: { id } })).name).toBe(
        'Admin Candidate',
      );
      expect(
        (await prisma.candidateAuth.findUniqueOrThrow({ where: { candidateId: id } })).passwordHash,
      ).toBe(auth.passwordHash);
    } finally {
      await prisma.$executeRaw`ALTER TABLE candidate_auth DROP CONSTRAINT test_reject_password`;
    }
  });
  ...
  it('filters company listing and replaces categories with the Other fallback', async () => {
    const list = await admin(
      api().get(
        '/api/v1/admin/companies?search=admin&category=NestJS&action=MONITOR_READY&pageSize=1',
      ),
    ).expect(200);
    expect(list.body.total).toBe(1);
    expect(list.body.rows[0].id).toBe('admin-main');
    await admin(api().put('/api/v1/admin/companies/admin-main'))
      .send({ name: ' Renamed ', active: false, categories: [] })
      .expect(200);
  ...
  it('rolls back company edits and deleted assignments when category insertion fails', async () => {
    ...
    try {
      await admin(api().put('/api/v1/admin/companies/admin-main'))
        .send({ name: 'Should roll back', active: false, categories: ['Other'] })
        .expect(500);
      expect(await prisma.company.findUnique({ where: { id: 'admin-main' } })).toEqual(before);
      const links = await prisma.companyCategory.findMany({
        where: { companyId: 'admin-main' },
        include: { category: true },
      });
      expect(links.map((link) => link.category.name)).toEqual(['NestJS']);
    } finally {
      await prisma.$executeRaw`ALTER TABLE company_categories DROP CONSTRAINT test_reject_assignment`;
    }
  });

  it('upserts category types and counts Other-only companies', async () => {
    await admin(api().post('/api/v1/admin/categories'))
      .send({ name: 'Admin Category', type: 'domain' })
      .expect(201);
    await admin(api().post('/api/v1/admin/categories'))
      .send({ name: 'Admin Category', type: 'sector' })
      .expect(201);
    const other = await prisma.category.findUniqueOrThrow({ where: { name: 'Other' } });
    await prisma.companyCategory.create({
      data: { companyId: 'admin-main', categoryId: other.id },
    });
    const response = await admin(api().get('/api/v1/admin/categories')).expect(200);
    expect(
      response.body.find((row: { name: string }) => row.name === 'Admin Category'),
    ).toMatchObject({ type: 'sector', companyCount: 0 });
    expect(response.body.find((row: { name: string }) => row.name === 'Other').companyCount).toBe(
      1,
    );
    await admin(api().post('/api/v1/admin/categories'))
  ...
  it('filters jobs and paginates tied discovery times with serialized IDs', async () => {
    ...
    const first = await prisma.job.create({
      data: {
        companyId: 'admin-main',
        title: 'Admin Engineer',
        jobHash: 'admin-job-one',
    ...
    const second = await prisma.job.create({
      data: {
        companyId: 'admin-main',
        title: 'Admin Engineer Two',
        jobHash: 'admin-job-two',
        first_seen_at: firstSeen,
        status: 'OPEN',
      },
    });
    await prisma.job.create({
      data: {
        companyId: 'admin-main',
        title: 'Admin Closed',
        jobHash: 'admin-job-closed',
        status: 'CLOSED',
      },
    });
    const page = await admin(
      api().get('/api/v1/admin/jobs?search=admin&status=OPEN&pageSize=1'),
    ).expect(200);
    expect(page.body).toMatchObject({ total: 2, page: 1, pageSize: 1 });
    expect(page.body.rows[0]).toMatchObject({
      id: second.id.toString(),
      companyName: 'Admin Alpha',
      companyCategories: ['NestJS'],
    });
    const next = await admin(
      api().get('/api/v1/admin/jobs?search=admin&status=OPEN&pageSize=1&page=2'),
    ).expect(200);
  ...
  it('excludes unlinked crawl logs and paginates tied timestamps deterministically', async () => {
    ...
    const first = await prisma.crawlLog.create({
      data: { company_id: 'admin-main', success: true, jobs_found: 2, checked_at: checked },
    });
    const second = await prisma.crawlLog.create({
      data: {
        company_id: 'admin-main',
        success: false,
        error: 'test timeout',
        checked_at: checked,
      },
  ...
  it('persists only editable settings, trims provider, and validates bounds', async () => {
    await prisma.setting.upsert({
      where: { key: 'admin_test_unrelated' },
      create: { key: 'admin_test_unrelated', value: 'preserve' },
      update: {},
    });
    ...
    expect(
      (await prisma.setting.findUniqueOrThrow({ where: { key: 'admin_test_unrelated' } })).value,
    ).toBe('preserve');
    await admin(api().put('/api/v1/admin/settings'))
      .send({ ...settingsInput, quickSearchDailyLimit: 21 })
      .expect(400);
    await admin(api().put('/api/v1/admin/settings'))
      .send({ ...settingsInput, unknown: true })
      .expect(400);
  });

  it('rolls back all settings when a later setting write fails', async () => {
    await admin(api().put('/api/v1/admin/settings'))
      .send({ ...settingsInput, emailEnabled: false, aiEnabled: false })
      .expect(200);
    ...
    try {
      ...
      await prisma.$executeRaw`ALTER TABLE settings DROP CONSTRAINT test_reject_email`;
    }
  });
});
