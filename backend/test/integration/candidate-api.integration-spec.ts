...
describe('Candidate and admin HTTP integration (mock database)', () => {
  ...
  const candidate = {
    ...
    auth: { passwordHash: '', mustChangePassword: true },
  };
  ...
  beforeEach(() => {
    ...
    prisma.candidate.findFirst.mockImplementation(async ({ select }) =>
      select
    ...
    prisma.candidateAuth.upsert.mockImplementation(async ({ update }) => {
      candidate.auth = update;
    });
  });
  afterAll(async () => {
    await app?.close();
  ...
  async function login() {
    const response = await request(app.getHttpServer())
      ...
      .send({ email: candidate.email, password: 'initial-password' })
      .expect(201);
  ...
  it('logs in, reads identity, changes the initial password, and clears the cookie', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(201, { ok: true });
    expect(await verifyLegacyScrypt('replacement-password', candidate.auth.passwordHash)).toBe(
      true,
    );
  ...
  it.each([undefined, 'ji_candidate_session=invalid', 'ji_candidate_session=%E0%A4%A'])(
    'rejects missing or malformed session cookies (%s)',
    async (cookie) => {
  ...
  it('rejects a session when the candidate is no longer active', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .set('Cookie', cookie)
      .expect(401);
    expect(prisma.candidate.findFirst).toHaveBeenLastCalledWith(
      expect.objectContaining({ where: { id: 42n, active: true } }),
    );
  });
  ...
  it.each([{ email: 'other@example.test' }, { candidateId: '99' }, { minimumMatchScore: 101 }])(
    'rejects profile identity changes and invalid score: %j',
    async (invalid) => {
      candidate.auth.mustChangePassword = false;
      const { cookie } = await login();
      await request(app.getHttpServer())
        .put('/api/v1/candidate/profile')
        .set('Cookie', cookie)
        .send({ name: 'Candidate', minimumMatchScore: 60, ...invalid })
        .expect(400);
  ...
  it('requires admin credentials independently of the candidate session', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect(200);
    await request(app.getHttpServer())
      .get('/api/v1/admin/companies?pageSize=10000')
      .auth('test-admin', 'test-admin-password')
      .expect(400);
  });
});
