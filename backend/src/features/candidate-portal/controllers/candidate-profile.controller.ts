...
export class CandidateProfileController {
  ...
  @Get()
  get(@Req() request: CandidateRequest) { return this.profiles.get(request.candidate.id); }
  @Put()
  async update(@Req() request: CandidateRequest, @Body() input: UpdateCandidateProfileDto) { await this.profiles.update(request.candidate.id, input); return { ok: true }; }
}
