...
export class AdminCandidatesService {
  ...
  private async ensurePassword(transaction: Prisma.TransactionClient, candidateId: bigint, explicit?: string): Promise<void> {
    if (explicit) { await this.authentication.setPassword(candidateId, explicit, true, transaction); return; }
    const auth = await transaction.candidateAuth.findUnique({ where: { candidateId }, select: { passwordHash: true } });
    if (!auth?.passwordHash) await this.authentication.setPassword(candidateId, this.config.app.defaultCandidatePassword, true, transaction);
  }

  private id(value: string): bigint {
    if (!/^[1-9]\d{0,18}$/.test(value) || BigInt(value) > 9223372036854775807n) {
      throw new BadRequestException({ code: 'INVALID_CANDIDATE_ID', message: 'Candidate ID is invalid.' });
    }
    return BigInt(value);
  }
