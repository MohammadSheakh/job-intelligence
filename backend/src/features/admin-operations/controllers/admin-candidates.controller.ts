...
export class AdminCandidatesController {
  ...
  @Get() list() { return this.candidates.list(); }
  @Get(':id') get(@Param('id') id: string) { return this.candidates.get(id); }
  @Post() create(@Body() input: SaveCandidateDto) { return this.candidates.create(input); }
  @Put(':id') async update(@Param('id') id: string, @Body() input: SaveCandidateDto) { await this.candidates.update(id, input); return { ok: true }; }
}
