...
export class UpdateCompanyDto {
  ...
  @IsOptional() @IsString() @MaxLength(2000) careerUrl?: string;
  @IsOptional() @IsString() @MaxLength(2000) linkedinUrl?: string;
  @IsOptional() @IsEmail() @MaxLength(320) email?: string;
  @IsOptional() @IsString() @MaxLength(500) location?: string;
  @IsOptional() @IsString() @MaxLength(4000) techStack?: string;
  @IsOptional() @IsString() @MaxLength(10000) notes?: string;
  @IsOptional() @IsIn(actions) recommendedAction?: typeof actions[number];
  @IsOptional() @IsString() @MaxLength(500) statusResearchHint?: string;
  @IsBoolean() active!: boolean;
  @IsArray() @IsString({ each: true }) @MaxLength(120, { each: true }) categories!: string[];
}
