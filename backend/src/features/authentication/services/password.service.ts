...
type ScryptAsync = (password: string | Buffer, salt: string | Buffer, keyLength: number, options: { N: number; r: number; p: number; maxmem: number }) => Promise<Buffer>;
...
export async function verifyLegacyScrypt(password: string, encoded: string): Promise<boolean> {
  try {
    ...
    const actual = await scrypt(password, Buffer.from(salt, 'hex'), expected.length, { ...parameters, N: Number(n), r: Number(r), p: Number(p) }) as Buffer;
