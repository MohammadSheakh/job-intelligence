# Database architecture — Job Intelligence

Project-specific schema ownership, tooling, and operational constraints. For reusable
persistence policy, read [database rules](../.agents/rules/backend-database.md). The separate
[multi-tenant reference](../.agents/rules/backend-database-multi-tenant.md) applies only to explicitly
...
[transaction rules](../.agents/rules/backend-transactions.md). Before connecting to a database,
read [database switching](./DATABASE_SWITCHING.md).

For comparison with the sibling project, read [Ferio tooling](FERIO_PRISMA_REFERENCE.md)
...
| `backend/prisma/schema.prisma` |  `backend/prisma/schema.prisma` is generated output.Never treat it as the authoritative edit location. If its diff changes without a corresponding fragment/builder change, investigate before accepting it. |
...
- Define the invariant before opening a transaction. Prefer nested writes when
  ...
  the global client. Follow [transaction rules](../.agents/rules/backend-transactions.md).
