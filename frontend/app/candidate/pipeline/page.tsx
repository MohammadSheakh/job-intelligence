    ...
    <main>
      <header>
        <div>
          ...
          <p className="muted">
            Track companies you plan to apply to, have applied to, or want to exclude.
          </p>
        </div>
        <nav>
        ...
        <section className="results">
          {items.map((item) => (
            ...
            >
              <div>
                ...
                <p className="tags">{item.categories.join(' · ') || 'Uncategorized'}</p>
              </div>
              <label>
                Status
              ...
              <label>
                Last applied
                <input name="lastAppliedAt" type="date" defaultValue={item.lastAppliedAt ?? ''} />
              </label>
              <label>
                Reapply count
                <input name="reapplyCount" type="number" min="0" defaultValue={item.reapplyCount} />
              </label>
              <label>
                Notes
                <textarea name="notes" defaultValue={item.notes ?? ''} maxLength={5000} />
              </label>
              <div>
                <button disabled={savingId === item.companyId}>
                  {savingId === item.companyId ? 'Saving…' : 'Save'}
                </button>
                <button
                  className="secondary"
                  type="button"
                  disabled={savingId === item.companyId}
          ...
          {items.length === 0 && (
            <section className="card">
              <p className="muted">
                Your pipeline is empty. Add a company through the candidate API while company
