...
export default function CandidateLoginPage() {
  ...
  useEffect(() => {
    ...
    api<{ enabled: boolean }>('/candidate-auth/google/status')
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await api<{ mustChangePassword: boolean }>('/candidate-auth/login', {
        method: 'POST',
        body: JSON.stringify({ email: form.get('email'), password: form.get('password') }),
      });
      router.push(result.mustChangePassword ? '/candidate/change-password' : '/candidate');
    } catch (reason) {
