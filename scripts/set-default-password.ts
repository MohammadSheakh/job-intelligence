...
if (!email)
  throw new Error('Usage: npm run candidate:set-default-password -- --email user@example.com');
...
try {
  ...
  await setCandidatePassword(Number(result.rows[0].id), env.defaultCandidatePassword, {
    mustChange: true,
  });
