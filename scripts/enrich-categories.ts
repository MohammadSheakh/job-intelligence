...
for (const company of companies) {
  const suggestions = suggestCategories(company).filter(
    (s) => s.confidence >= threshold && s.name !== 'Other',
  );
  ...
  for (const s of suggestions)
    console.log(`  - ${s.name} ${(s.confidence * 100).toFixed(0)}% (${s.reason})`);
  if (apply)
    await addCompanyCategories(
      company.id,
...
console.log(
  apply ? 'Applied suggestions.' : 'Dry run only. Re-run with --apply to write assignments.',
);
