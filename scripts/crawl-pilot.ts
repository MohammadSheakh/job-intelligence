...
for (const row of rows.slice(0, 5)) {
  ...
  if (!url) continue;
  try {
    const response = await fetch(url, { redirect: 'follow', headers: { 'user-agent': 'JobIntelligenceBot/0.1 (+career-monitoring)' }, signal: AbortSignal.timeout(15000) });
    ...
    console.log(JSON.stringify({ company, url, status: response.status, candidates: unique.slice(0, 10), pageHash: crypto.createHash('sha256').update(html).digest('hex').slice(0, 12) }, null, 2));
  } catch (error) {
    console.error(JSON.stringify({ company, url, error: error instanceof Error ? error.message : String(error) }));
  }
}
